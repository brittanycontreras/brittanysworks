...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            if place.ant.can_contain(self): #if occupying can contain new ant
                place.ant.store_ant(self) # store new ant in occupying ant

            elif self.can_contain(place.ant): #if new ant can contain occupying ant
                self.store_ant(place.ant) #store occupying ant in new ant
                place.ant = self #remove occupying ant and add new ant (now containing occupying ant)

            else:                      # if neither ant can contain the other, raise an error
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        curr_place, curr_dist = self.place, 0
        while not curr_place.is_hive:
            if curr_place.bees and self.min_range <= curr_dist <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        if amount >= self.health: #if ant dies
            amt_dam = amount + self.damage
            for bee in list(self.place.bees):
                Insect.reduce_health(bee, amt_dam)
...
# BEGIN Problem 6
class WallAnt(Ant):
    """The wall ant takes no action each turn, but has a large starting health value."""
    name = 'Wall'
    is_implemented = True
    food_cost = 4
...
class TankAnt(BodyguardAnt):
    """TankAnt is a container ant that protects another ant in its place 
...
class NinjaAnt(Ant):
    ...
    # BEGIN Problem Optional 1
    "*** YOUR CODE HERE ***"
    is_implemented = False   # Change to True to view in the GUI
    # END Problem Optional 1
